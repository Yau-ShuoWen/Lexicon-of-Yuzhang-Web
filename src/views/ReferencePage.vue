...
const cachedDictionary = {
  get: () => {
    const cached = localStorage.getItem('selectedDictionary')
    return cached ? JSON.parse(cached) : null
  },
  set: (code, name) => {
    localStorage.setItem('selectedDictionary', JSON.stringify({code, name}))
  },
  clear: () => localStorage.removeItem('selectedDictionary')
}
  ...
  async getDictionaries() {
    try {
      ...
      if (apiResponse.success !== undefined) {
        if (!apiResponse.success) throw new Error(apiResponse.message || '获取字典列表失败')
      ...
      throw new Error(`获取词典列表失败: ${error.message}`)
    }
  },
...
const processDictionaries = (data) => {
  ...
  return data.map(item => {
    ...
    else if (item.name !== undefined && item.code !== undefined) return {left: item.name, right: item.code}
    else if (typeof item === 'string') return {left: item, right: item}
    else return {left: '未知', right: 'unknown'}
  })
}
...
const loadDictionaries = async () => {
  try {
    ...
    const processedData = processDictionaries(data)
    dictionaries.value = processedData

    if (processedData.length > 0) {
      const cached = cachedDictionary.get()
      if (cached) {
        const cachedDict = processedData.find(dict => dict.right === cached.code)
        if (cachedDict) {
          searchForm.dictionary = cachedDict.right
          selectedDictionaryName.value = cachedDict.left
          ElMessage.success(`已恢复词典选择: ${cachedDict.left}`)
          return
        }
      }
      searchForm.dictionary = processedData[0].right
      selectedDictionaryName.value = processedData[0].left
      cachedDictionary.set(processedData[0].right, processedData[0].left)
    } else {
...
const handleDictionaryChange = (selectedCode) => {
  ...
  if (selectedDict) {
    ...
    cachedDictionary.set(selectedDict.right, selectedDict.left)
    ...
    <div class="search-section">
      <el-card>
        ...
        <el-form :model="searchForm" inline>
          ...
          <el-form-item label="词典选择" required>
            <el-select v-model="searchForm.dictionary" placeholder="请选择词典" style="width: 200px"
                       @change="handleDictionaryChange" :loading="dictionaries.length === 0">
              <el-option v-for="dict in dictionaries" :key="dict.right" :label="dict.left" :value="dict.right"/>
            </el-select>
          </el-form-item>
